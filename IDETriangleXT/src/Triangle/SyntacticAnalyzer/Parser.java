...
///////////////////////////////////////////////////////////////////////////////
  ...
  public Program parseProgram() {
    ...
    try {
      ...
      //NUEVO:
      if (currentToken.kind != Token.EOT && currentToken.kind != Token.NIL) {
...
// to represent its phrase structure.
    ...
    case Token.IF:
      {
        ...
        accept(Token.END);
        finish(commandPos);
        commandAST = new IfCommand(eAST, c1AST, orBody ,c2AST, commandPos);
      }
      break;
    
    //NUEVO:
    //Maneja los tipos de loop
    case Token.LOOP:
      {
        acceptIt();
        switch(currentToken.kind){
            ...
            case Token.WHILE:
                ...
                finish(commandPos);
                commandAST = new LoopCommand(whileBody, commandPos); 
                break;
            ...
            case Token.UNTIL:
                ...
                finish(commandPos);
                commandAST = new LoopCommand(untilBody, commandPos);
                break;
            ...
            case Token.DO:
                ...
                finish(commandPos);
                commandAST = new LoopCommand(doBody, commandPos);
                ; break;
            ...
            case Token.FOR:
                ...
                finish(commandPos);
                commandAST = new LoopCommand(forBody, commandPos);
                ; break;
                
            default:
                syntacticError("Found: \"%\", Expected while, until, do or for",
        currentToken.spelling);
  ...
  OrBody parseOrBody() throws SyntaxError{
    OrBody orBodyAST = null; // in case there's a syntactic error
    ...
    while (currentToken.kind == Token.PIPE) {
      ...
      finish(commandPos);
      orBodyAST = new OrBody(orBodyAST, expr, command, commandPos);
    }
    return orBodyAST;
  }
  ...
  WhileBody parseWhileBody() throws SyntaxError{
     ...
     accept(Token.END);
     finish(commandPos);
     whileBodyAST = new WhileBody(exprAST, commandAST, commandPos);
  ...
  ForBody parseForBody() throws SyntaxError{
     ...
     Identifier identAST = parseIdentifier();
     
     accept(Token.FROM);
     
     Expression exprAST1 = parseExpression();
     
     accept(Token.TO);
     
     Expression exprAST2 = parseExpression();
     
     Expression exprAST3 = null;
     
     //#2 NUEVO: Agregado un tipo de for omitido anteriormente
     
     if(currentToken.kind == Token.WHILE){
         accept(Token.WHILE);
         exprAST3 = parseExpression();
     }
     else if(currentToken.kind == Token.UNTIL){
         accept(Token.UNTIL);
         exprAST3 = parseExpression();
     ...
     forBodyAST = new ForBody(identAST, exprAST1, exprAST2, exprAST3, commandAST, commandPos);
...
///////////////////////////////////////////////////////////////////////////////
  ...
  Declaration parseCompoundDeclaration() throws SyntaxError{
    ...
    switch(currentToken.kind){
        case Token.REC:
            acceptIt();
            start(declarationPos);
            Declaration procFuncAST1 = parseProcFuncs();
            accept(Token.END);
            finish(declarationPos);
            declarationAST = new RecDeclaration(procFuncAST1, declarationPos);
            break;
        ...
        case Token.LOCAL:
            ...
            accept(Token.IN);
            Declaration declarationAST3 = parseDeclaration();
            accept(Token.END);
            finish(declarationPos);
            declarationAST = new LocalDeclaration(declarationAST2, declarationAST3, declarationPos);
            break;
  ...
  Declaration parseProcFuncs() throws SyntaxError{
      ...
      start(pos);
    //Deben haber minimo 2 procFuncs
    
    //Primero
    Declaration declaration = parseProcFunc();
    accept(Token.PIPE);
    //Segundo
    Declaration declaration2 = parseProcFunc();
    finish(pos);
    Declaration procFuncs = new ProcFuncs(declaration, declaration2, pos);
    
    while(currentToken.kind == Token.PIPE){
        acceptIt();
        declaration2 = parseProcFunc();
        finish(pos);
        procFuncs = new ProcFuncs(declaration2, procFuncs, pos);
    }
  ...
  Declaration parseProcFunc() throws SyntaxError{
    ...
    if(currentToken.kind == Token.PROC){
        acceptIt();
        declarationAST = parseProcBody();
    }
         
    
    else if(currentToken.kind == Token.FUNC){
        acceptIt();
        declarationAST = parseFuncBody();
    }
         
    
    else
         syntacticError("Found \"%\" when expecting proc or func", currentToken.spelling);
...
//-------------
  ...
  // NUEVO:
  Declaration parseProcBody() throws SyntaxError{
    Declaration declarationAST = null; // in case there's a syntactic error
  ...
  Declaration parseFuncBody()throws SyntaxError{
    Declaration declarationAST = null; // in case there's a syntactic error
